...
public class GameManager {
    ...
    private final Set<Material> allowedDisguiseBlocks = EnumSet.noneOf(Material.class);
    ...
    public void reloadFromConfig() {
        ...
        if (selectedArena != null) {
            ...
            cacheArenaBlockPool(selectedArena);
            currentArenaBlockPool.clear();
            currentArenaBlockPool.addAll(selectedArena.getCachedBlockPool());
    ...
    public void startGame(CommandSender sender) {
        ...
        GameMessageUtils.sendPrefixedChat(
            matchAndAdmins,
            prefix,
            msg("messages.game.seekers-list", "&cSeekers this round: {seekers}",
                Map.of("seekers", seekerNames.isEmpty() ? "Unknown" : String.join(", ", seekerNames))));
            ...
            if (data.isLocked()) {
                if (data.getPlacedBlockLocation() != null) {
                    Location anchor = data.getLockedPlayerLocation() != null
                            ...
                            : data.getPlacedBlockLocation().clone().add(0.5, 1.001, 0.5);
    ...
    private void lockHider(Player p, HiderData data) {
        ...
        p.setFlying(false);
        p.setGravity(false);
        Location lockTp = resolveLockedPlayerLocation(anchor, p, chosenBlock);
    ...
    private Location getFloorAnchorLocation(Player p, Material chosenBlock) {
        ...
        int anchorY = floorY + 1;
        if (hasNarrowCollisionAdjacent(world, x, anchorY, z))
            return null;
        if (needsVerticalSpace && hasNarrowCollisionAdjacent(world, x, anchorY + 1, z))
            return null;
    ...
    private boolean hasNarrowCollisionAdjacent(World world, int x, int y, int z) {
        return isNarrowCollisionBlock(world.getBlockAt(x + 1, y, z).getType())
                || isNarrowCollisionBlock(world.getBlockAt(x - 1, y, z).getType())
                || isNarrowCollisionBlock(world.getBlockAt(x, y, z + 1).getType())
                || isNarrowCollisionBlock(world.getBlockAt(x, y, z - 1).getType());
    }

    private boolean isNarrowCollisionBlock(Material material) {
        if (material == null)
            return false;

        return material == Material.IRON_BARS
                || isPaneFamily(material)
                || Tag.WALLS.isTagged(material)
                || isFenceFamily(material);
    }
    ...
    private Location resolveLockedPlayerLocation(Location anchor, Player player, Material chosenBlock) {
        Location defaultLocation = anchor.clone().add(0.0, 1.001, 0.0);
        if (chosenBlock == null || player == null || anchor.getWorld() == null)
            return defaultLocation;
        if (!Tag.DOORS.isTagged(chosenBlock))
            return defaultLocation;

        BlockFace doorFront = resolveHorizontalFace(player.getLocation().getYaw());
        BlockFace behind = doorFront.getOppositeFace();

        Location behindLocation = anchor.clone().add(
                behind.getModX() * 0.62,
                0.001,
                behind.getModZ() * 0.62);
        if (isPlayerSpaceFree(behindLocation)) {
    ...
    private BlockFace resolveHorizontalFace(float yaw) {
    ...
    private boolean isPlayerSpaceFree(Location feetLocation) {
        if (feetLocation == null || feetLocation.getWorld() == null)
            return false;

        World world = feetLocation.getWorld();
        Material feet = world.getBlockAt(feetLocation).getType();
        Material head = world.getBlockAt(feetLocation.clone().add(0, 1, 0)).getType();
        return isPassableForPlayer(feet) && isPassableForPlayer(head);
    }

    private boolean isPassableForPlayer(Material material) {
        if (material == null)
            return false;
    ...
    public void handleHiderHit(Player seeker, Interaction hitbox) {
        if (state != State.SEEKING_PHASE)
            return;
        UUID targetUUID = getDisguiseEntityOwner(hitbox.getUniqueId(), hitbox);
        if (targetUUID == null)
            return;
        applyHiderHit(seeker, targetUUID);
    }

    private UUID getDisguiseEntityOwner(UUID entityId, org.bukkit.entity.Entity entity) {
        UUID hiderId = disguiseEntityOwners.get(entityId);
        if (hiderId == null)
            return null;
        ...
        disguiseEntityOwners.remove(entityId);
        return null;
    }
    ...
    private void removePlayerFromCollisionTeams(Player player) {
        ...
        String entry = player.getName();
        Team hiderTeam = getCollisionTeam(HIDER_NO_COLLISION_TEAM);
        Team seekerTeam = getCollisionTeam(SEEKER_NO_COLLISION_TEAM);
        if (hiderTeam != null)
            hiderTeam.removeEntry(entry);
        if (seekerTeam != null)
            seekerTeam.removeEntry(entry);
    ...
    private Team ensureNoCollisionTeam(String teamName) {
        Team team = getCollisionTeam(teamName);
        if (team == null) {
    ...
    private Team getCollisionTeam(String teamName) {
        ScoreboardManager scoreboardManager = Bukkit.getScoreboardManager();
        if (scoreboardManager == null)
            return null;
    ...
    private void unregisterCollisionTeams() {
        for (String teamName : List.of(HIDER_NO_COLLISION_TEAM, SEEKER_NO_COLLISION_TEAM)) {
            Team team = getCollisionTeam(teamName);
            if (team != null)
                team.unregister();
        }
    ...
    private String setArenaPos(Player player, String name, String posKey) {
        ...
        plugin.saveArenaConfig();
        arenaRuntimeCache.remove(result.key());
    ...
    private ArenaRuntime getOrCreateArenaRuntime(ArenaUtils.ArenaDefinition loaded) {
        ArenaRuntime cached = arenaRuntimeCache.get(loaded.name());
        if (cached != null
                && Objects.equals(cached.world, loaded.world())
                && Objects.equals(cached.pos1, loaded.pos1())
                && Objects.equals(cached.pos2, loaded.pos2())
                && Objects.equals(cached.hiderSpawns, loaded.hiderSpawns())
                && Objects.equals(cached.seekerSpawns, loaded.seekerSpawns())) {
            return cached;
        }

        ArenaRuntime arena = new ArenaRuntime(
                loaded.name(),
                loaded.world(),
                loaded.pos1(),
                loaded.pos2(),
                loaded.hiderSpawns(),
                loaded.seekerSpawns());
        arenaRuntimeCache.put(loaded.name(), arena);
        return arena;
    }
    ...
    private List<ArenaRuntime> loadArenasFromConfig() {
        ...
        if (loadedArenas.isEmpty()) {
            arenaRuntimeCache.clear();
            return Collections.emptyList();
        }

        List<ArenaRuntime> arenas = new ArrayList<>();
        for (ArenaUtils.ArenaDefinition loaded : loadedArenas) {
            ArenaRuntime arena = getOrCreateArenaRuntime(loaded);
            arenas.add(arena);
            cacheArenaBlockPool(arena);
        }

        for (Iterator<Map.Entry<String, ArenaRuntime>> it = arenaRuntimeCache.entrySet().iterator(); it.hasNext(); ) {
            Map.Entry<String, ArenaRuntime> entry = it.next();
            if (loadedArenas.stream().noneMatch(loaded -> loaded.name().equals(entry.getKey()))) {
                it.remove();
            }
        }
    ...
    private boolean isAllowedPropMaterial(Material mat) {
        return allowedDisguiseBlocks.contains(mat);
    }

    private boolean isConfiguredPropMaterial(Material mat) {
        if (mat == null)
            return false;
    ...
    private void rebuildAllowedDisguiseBlocks() {
        allowedDisguiseBlocks.clear();
        for (Material material : Material.values()) {
            if (isConfiguredPropMaterial(material))
                allowedDisguiseBlocks.add(material);
        }
    }
    ...
    private ItemStack createSeekerVisualWeapon() {
        ...
        if (meta != null) {
            meta.displayName(Component.text("§6Seeker Sword"));
    ...
    private boolean isSeekerVisualWeapon(ItemStack item) {
        ...
        ItemMeta meta = item.getItemMeta();
        return meta != null && matchesItemDisplayName(meta, SEEKER_WEAPON_NAME) && meta.isUnbreakable();
    }
    ...
    public boolean isLockedHiderMenuItem(ItemStack item) {
        return isHiderMenuItem(item);
    }

    public int getHiderMenuSlot() {
        return HIDER_MENU_SLOT;
    }
    ...
    public void setHiderBlock(Player p, Material block) {
        setHiderBlockInternal(p, block, true);
    }

    private boolean setHiderBlockInternal(Player p, Material block, boolean sendDisguisedAsMessage) {
        if (p == null)
            return false;
    ...
    private boolean matchesItemDisplayName(ItemMeta meta, String expectedName) {
        ...
        return expectedName.equals(plain);
    }
    ...
    private void tagHiderCombat(Player hider) {
        if (hider == null || hiderCombatTagSeconds <= 0)
            return;
    ...
    private boolean isHiderCombatTagged(Player hider) {
        ...
        Long until = hiderCombatTagUntil.get(hider.getUniqueId());
        if (until == null)
            return false;
        if (System.currentTimeMillis() <= until)
            return true;

        hiderCombatTagUntil.remove(hider.getUniqueId());
        return false;
    }

    private int getHiderCombatTagSecondsRemaining(Player hider) {
        if (hider == null)
            return 0;
        ...
        long millisRemaining = Math.max(0L, until - System.currentTimeMillis());
    ...
    private ItemStack createHiderMenuItem() {
        ...
        ItemMeta meta = item.getItemMeta();
        if (meta != null) {
            meta.displayName(Component.text(HIDER_MENU_ITEM_NAME));
            meta.setUnbreakable(true);
            meta.addItemFlags(ItemFlag.HIDE_ATTRIBUTES, ItemFlag.HIDE_UNBREAKABLE);
            item.setItemMeta(meta);
        }
    ...
    private boolean isInBlockSelectionMenu(Player player) {
        ...
        String plain = PlainTextComponentSerializer.plainText().serialize(player.getOpenInventory().title());
        return plain.startsWith("Pick Your Block");
    }
